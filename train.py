...
class dataset(torch.utils.data.Dataset):
    def __init__(self, data_path):
        self.imgs = list()
        self.transform = transforms.Compose([transforms.ToPILImage(),transforms.Resize((64, 64)),transforms.RandomHorizontalFlip(),transforms.ToTensor(),])

        count = 0

        for i in os.listdir(data_path)[:400]:
            if i.endswith(".npz"):
                self.data = np.load(os.path.join(data_path, i))["obs"]

            for j in range(self.data.shape[0]):
                print(f"Loaded {i} file, {count} images.            ", end="\r")
                count += 1

                self.imgs.append(self.data[j])
...
if __name__ == '__main__':
    ...
    train_ds = dataset("/dataset/imgs")
    print(len(train_ds))
    train_dataloader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=opt.n_cpu, drop_last=True)
    ...
    #model.load_state_dict(torch.load("/home/pagadalam/Santhosh/Workspace/Fake_Assignments/world-models-nvae/nvae/checkpoints/ae_ckpt_28_0.902669.pth", map_location=device), strict=False)
    ...
    for epoch in range(epochs):
        ...
        for i, image in enumerate(train_dataloader):
            ...
            # print(image.shape)
            image_recon, recon_loss, kl_losses, m, _ = model(image)
            # print(image_recon.shape, m.shape)
