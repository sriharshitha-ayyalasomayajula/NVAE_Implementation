...
class Decoder(nn.Module):
    ...
    def forward(self, z, xs=None, mode="random", freeze_level=-1):
        ...
        :return:
        """
        # increase z dimension from 32x1x1 to 32x2x2
        z = z.repeat(1, 1, 2, 2)
        ...
        for i in range(len(self.decoder_residual_blocks)):
            ...
            # print("At i:{} z.shape :{} {}".format(i,z.shape,decoder_out.shape))
            z_sample = torch.cat([decoder_out, z], dim=1)
            # print(" At i:{} z_sample.shape :{}".format(i,z_sample.shape))
            ...
            if xs is not None:
                # print("xs[i].shape", xs[i].shape,decoder_out.shape)
